...
Alliances reflect ranges of diagnostic and dominant species and physiognomy organized along gradients of environmental drivers at regional to subregional ecological scales (Faber-Langendoen et al. 2014). To ensure that we conducted clustering and ordination at an eco-spatial scale appropriate to alliances, we defined 18 subregions corresponding to high density sampling areas based on unique combinations of vegetation regions, Major Land Resource Areas (MLRAs; NRCS 2025), unified ecoregions (Nowacki et al. 2001), or agency jurisdictions (e.g., Inventory and Monitoring data collected by National Park Service personnel in Denali National Park and Preserve; Roland et al. 2021). We then queried detailed vegetation composition data appropriate to clustering and ordination (i.e., benchmark datasets) from the AKVEG Database for each subregion according to the following criteria: 1) the site was surveyed from the ground perspective, 2) cover measurements were absolute foliar or canopy cover, and 3) observers targeted exhaustive or non-trace species lists for at least vascular plants (it is uncommon in Alaska for surveys to include complete species lists for bryophytes and lichens). We merged vascular plant observations into species (or genera where species were not determined) and bryophyte and lichen observations into genera (or functional groups where genera were not determined). For each subregion that included ≥ 800 site visits, we further split the corresponding site visits into “forested” (tree cover >= 10%) and “non-forested” (tree cover < 10%) partitions to reduce the computational workload of subsequent clustering and ordination operations and to reduce stress in the ordinations.

To ensure statistical independence between the foliar cover predictions and the ordination and clustering analyses, we extracted predicted cover values for benchmark sites from the merged test partitions of the outer cross-validation iterations. For site visits that did not contribute to the development of a particular diagnostic species map and were therefore statistically independent from model training and validation, we instead extracted the predicted cover value from the raster map. Thus, all foliar cover predictions that we used to evaluate combined performance were statistically independent from the vegetation composition data that we used in ordination and clustering analyses. We compared the combined performance of the stack of foliar cover maps to combined performances calculated for the fine classes (i.e., the most detailed classes) of the Alaska Vegetation and Wetland Composite (AKVWC; Flagstad et al. 2018) and the LANDFIRE 2023 Existing Vegetation Types (EVT; La Puma 2023). Therefore, we also extracted the map classes from the AKVWC and LANDFIRE 2023 EVT to the site visits from benchmark datasets and converted memberships for each map class to individual variables with Boolean values.

We conducted two stages of clustering on composition matrices of normalized taxa abundances for two different purposes: 1) to remove data that caused high stress (i.e., lower reliability) in the ordinations, and 2) to develop the preliminary alliances and assign membership to site visits. We initially conducted fuzzy noise clustering (Davé and Krishnapuram 1997) across a range of sequential cluster numbers up to 20 and manually selected the best-performing set of clusters based on our interpretation of the mean cluster silhouette width and mean (within) cluster variance. Based on the best-performing number of clusters, we removed site visits with ≥ 80% membership to the noise class. To develop preliminary alliances, we compared hard c-medoid (Krishnapuram et al. 1999) clustering solutions across a range of sequential cluster numbers up to 52. Similar to our process with noise clustering, we manually selected the best-performing set of clusters based on mean silhouette width and mean variance. We relied on the hard c-medoid method rather than solely using fuzzy noise clustering to maintain computational efficiency for large numbers of site visits when the cluster number exceeded 20. Initial methodological experimentation (results not reported) revealed that deviance explained by preliminary alliances was not highly sensitive to the selection of clustering algorithm or cluster number among multiple options with relatively good combinations of mean silhouette width and mean variance.
